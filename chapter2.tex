...
%Cluster (CC).
Our goal in this study was to improve the famous Coupled Cluster 
methods. Since their introduction in 
the nuclear physics,\cite{coester1958bound, coester1960short} Coupled Cluster
...
important caveat is that, however, that while keeping the dimension of the 
auxiliary basis low, in this approximation one can only 
...
$r_{RI} = O(N)$ in the former case to provide equivalent accuracy. The 
approximation in the last case would be not useful, because RI factors would 
contain almost the same number of elements as the original tensor. Those 
...
The polyadic decomposition of a tensor is a decomposition of 
...
decomposition. As the rank is increased, this decomposition can approximate an 
arbitrary tensor with any given precision. The factorization with the lowest 
possible rank is called canonical polyadic decomposition (CPD), but we would 
omit this detail and call CPD any decomposition of the form shown in 
...
techniques to calculate the CP decomposition.\cite{sorber2013optimization} 
...
$r_{T}$ and $N$ per iteration. An example of such factorization 
